...
| HTML 安全预览 | 使用 WKWebView 渲染本地 HTML，默认禁用 JavaScript，阻断外部 http/https 资源，并限制外部跳转 | P0 |
...
| 付费下载 | MVP 采用付费下载，不做 IAP、购买页、恢复购买或功能 gating | P0 |
| 内置示例 | 首页提供 HTML、Markdown、ZIP 三个示例，方便首次体验与 App Review | P0 |
| 自动化测试 | 覆盖 ZIP 安全、文件类型、入口识别、metadata、失败清理、HTML 安全策略、删除清理 | P0 |
...
| 购买状态 | 不显示 | MVP 为付费下载，无 App 内购买状态 |
| 恢复购买 | 不适用 | MVP 不集成 IAP |
| 隐私说明 | 常驻 | 明确“不上传文件” |
...
- 禁用 JavaScript
- 通过 WKContentRuleList 或等价资源拦截阻断外部 `http://` / `https://` 资源请求
...
1. 安全模式不降级文案，继续承诺“默认不加载外部资源”。
2. 资源阻断属于 P0，不能只依赖 `WKNavigationDelegate`。
3. `WKNavigationDelegate` 用作跳转、表单提交、外部 scheme 的第二层防护。
4. WebKit 数据存储默认使用 non-persistent。
...
3. 仅通过 navigation delegate 阻止外部链接不足以覆盖资源请求；MVP 必须使用 WKContentRuleList 或等价资源拦截策略阻断外部网络资源。
...
MVP 推荐使用 SwiftUI 原生视图渲染 Markdown，而不是把 Markdown 全部转成 HTML 再交给 WebView。MVP 定义为“常见 Markdown 基础阅读支持”，不承诺完整 GFM。理由：
...
- 表格、任务列表、脚注、Mermaid、LaTeX、复杂 HTML 混排放到 P1 或以后。
...
HTML 和 Markdown 的相对资源规则保持一致：单文件导入时相对资源是 best effort；ZIP 包导入才是可靠方案。
...
- 不能出现重复规范化路径
- 不能出现大小写冲突路径
- 需要过滤 `__MACOSX` 与 `.DS_Store`
- 解压后总大小不能超过限制
- 文件数量不能超过限制
- 单文件大小不能超过限制
- 需要防御解压炸弹
- 入口文件必须是 html / htm / md / markdown
- 必须先解压到临时目录，成功校验后再移动到正式导入目录
- 失败或取消时必须清理临时目录
```
...
- 不实现 `PurchaseManager`
- 不做购买页、恢复购买、功能 gating
- 不做 IAP 沙盒测试
- 产品页、截图和隐私说明必须清楚表达“付费下载、无订阅、无广告、无账号”

### 11.2 后续若采用免费 + 买断

后续如果要做免费试用版，再单独评估迁移方案。免费版建议不要用广告，不做烦人的弹窗。可以限制高级功能，而不是限制基础打开能力。
...
最低支持系统：iOS 17。除非 M0 技术验证发现某个关键实现必须依赖 iOS 18-only API，否则不提高到 iOS 18。
...
> 请查看产品页文案和截图，告诉我们你理解这个付费方式吗？你认为它是订阅、广告移除，还是付费下载？

观察点：

- 是否明确知道“无订阅”
- 是否明确知道“无广告”
- 是否理解这是付费下载 App，不是免费试用或订阅
- 价格接受度
...
如果用户误以为付费下载是订阅，产品页和 App 内说明必须把“无订阅”放在标题或价格旁边。
...
1. MVP 采用付费下载，不做 IAP。
2. MVP 最低系统定为 iOS 17。
3. Markdown MVP 采用原生基础阅读支持，不承诺完整 GFM。
4. ZIP 是 MVP 核心卖点。
5. 内置 HTML、Markdown、ZIP 示例进入 P0。
6. HTML 安全模式必须默认阻断外部 http/https 资源。
7. PDF 导出不进入首发 P0。
8. iPad 双栏布局保持 P1，首发只做基础适配。

### 21.2 M0 验证重点

1. Document Types / UTType 能否让 App 稳定出现在文件 App、微信、邮件的“打开方式”里。
2. WKWebView 安全模式是否能真正阻断外部 http/https 资源，而不仅是阻止跳转。
3. ZIP 解压后用入口 HTML + 解压根目录加载，CSS 和图片能否稳定显示。

### 21.3 仍需 M0 后确认

1. ZIP 文件大小、总解压大小、文件数和单文件大小最终限制是否需要根据性能测试调整。
2. 首页是否直接放“从微信打开教程”，等待入口可用性测试后决定。
