...
- App marketing version is set to `1.1` and build number is `4`.
- Release simulator build verifies `CFBundleShortVersionString=1.1`, `CFBundleVersion=4`, app icon assets, and privacy manifest in the app bundle.
...
- Use the current `DerivedData/PhysicalDeviceValidationRun/.../physical-device-validation-result.md` draft generated by `scripts/final-submission-preflight.sh`, or run `scripts/prepare-physical-device-validation-run.sh --device <physical-iPhone>` immediately before the physical-device source matrix to prefill a device-specific result draft and sample packet.
