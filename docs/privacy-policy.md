# HTML Previewer Privacy Policy

Effective date: 2026-06-28

HTML Previewer is a local file viewer for HTML, Markdown, and ZIP report packages.

## Data Collection

HTML Previewer does not collect personal data.

The app does not use analytics SDKs, advertising SDKs, tracking SDKs, account systems, subscriptions, or in-app purchases.

## File Processing

Files selected by the user are copied into the app sandbox so they can be previewed locally and reopened from the recent files list.

The app processes selected files on device. User files are not uploaded to a server by the app.

## Network Access

Safe Preview is the default HTML preview mode. In Safe Preview, JavaScript is disabled and external HTTP/HTTPS resources are blocked.

Interactive mode is available for trusted local files. When a user chooses to use Interactive mode for a trusted file, that file's own content may request external resources according to WebKit and iOS behavior.

## Storage And Deletion

Imported files and document metadata are stored locally in the app container.

Deleting a recent file in the app removes the app's local copy for that item.

Deleting the app removes the app container and its locally stored files.

## Children

The app is not directed to children and does not knowingly collect data from children.

## Contact

For support or privacy questions, use the support page:

https://github.com/Kaedeeeeeeeeee/ios-html-markdown-previewer/blob/main/docs/support.md

## Changes

If this policy changes, the updated policy will be published at this URL.
